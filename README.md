...
2. **X stitches** — two crossing diagonal lines per filled cell, printed on top of the mesh
   ...
   color the printer pauses (`M400 U1`) — unload and load the next color from the printer
   screen's filament menu, then resume. An `M600` firmware-change-routine mode is available
...
3. At each **pause**, the screen shows which color to load. Open the filament menu, unload,
   load the next color, purge until it runs clean, then **resume**. A short purge line is also
